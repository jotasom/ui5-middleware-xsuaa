        ...
        try {
            ...
            switch (this.grantType) {
                case "authorizationCode":
                    ...
                    if (this.redirectUri) {
                        path += "&redirect_uri=" + this.redirectUri;
                    }
...
class XsuaaServiceInstance extends OAuth2Service {
    ...
    constructor(grantType) {
        const config = findBoundService(service => {        
            return service.label === "xsuaa";
        });

        if (!config) {
            throw Error(`Xsuaa service not bound.`);
        }
...
class Destination extends OAuth2Service {
    ...
    async authorize() {
        ...
        if (this._xsuaa) {
            this._xsuaa.authorizationCode = this.authorizationCode;
            await this._xsuaa.authorize();
            headers["x-user-token"] = this._xsuaa.accessToken;
        }
            ...
            const request = https.request({
                ...
                response.on("end", () => {
                    ...
                    if (response.statusCode !== 200) {
                        ...
                        if (!access_token) {
                            let error = json.authTokens.find(token => token.error);
                            if (error) {
                                this.authorizationError = error.error;
                            } else {
                                this.authorizationError = `Destination service does not return a valid OAuth2 bearer token to use with this destination.`;
                            }
